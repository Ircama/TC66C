...
def handleDataPacket(data):
    ...
    print(
        f"V: {voltage}\tI: {current}\tW: {power}\tΩ: {ohms}\tmAh: {mAh0}\tmWh: {mWh0}\tmAh: {mAh1}\tmWh: {mWh1}\tTemp: {temperature}\tD+: {dataPlus}\tD-: {dataMinus}"
    )
...
def decodeDataBuffer(IncomingDataBuffer):
    while len(IncomingDataBuffer) >= 192:
        ...
        if (
            decodedData[0] == 112
            and decodedData[1] == 97
            and decodedData[2] == 99
            and decodedData[3] == 49
        ):
    ...
    with Input(keynames="curtsies", sigint_event=True) as input_generator:
        while True:
            ...
            if key and (
                ...
                print("Terminated")
                break
            if key and key in ("r", "R", "<UP>", "<DOWN>"):
                devWriteChar.write(rotateScreen, True)
            if key and key in ("<RIGHT>", "1"):
                devWriteChar.write(BackScreen, True)
            if key and key in ("<LEFT>", "2"):
                devWriteChar.write(ForwScreen, True)

            devWriteChar.write(
                AskForData, True
            )  # Ask for data to be sent in a notification
            while len(IncomingDataBuffer) < 192:
